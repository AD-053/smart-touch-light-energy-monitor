// Pin Configurations
const int touchPin = 2; // CHANGE 1: Switched to pin 2
const int ledPin = 13;

// State Variables
bool ledState = false;
bool lastTouchState = HIGH; // CHANGE 3b: Initialized to HIGH for internal pull-up logic
...
void setup() {
  pinMode(touchPin, INPUT_PULLUP); // CHANGE 2: Using internal pull-up resistor
...
void loop() {
  // 1. Handle Automatic Report (Temporarily set to 60000UL / 1 minute for fast testing)
  if (millis() - twentyFourHourTimer >= 60000UL) { 
    printReport("Interval Monitoring Completed");
    resetStatistics();
    Serial.println("Monitoring Session Completed.");
  ...
  // CHANGE 3a: Reversed logic (LOW = Pressed, HIGH = Released)
